...
En annexe\ref{app:testLiDAR}, se trouve le protocole de test complet du capteur.
...
Comme cette grande erreur aplatit totalement les mesures sous 3.5m, nous avons jugé important d'effectuer un
...
être compensée. De plus, dans le projet, nous ne travaillons qu'avec des différences entre deux mesures, ce qui limite d'autant 
plus les effets de cette erreur.\par

Nous pouvons finalement conclure que ce test est réussi. En effet, malgré une erreur non-négligeable de mesure,
...
section \ref{sec:banc}. Le but final est de générer du bruit de mesure afin de représenter au mieux une
...
au fil des séries, représenté par la longueur des barres d'erreur. Comme l'indiquent les boîtes
à moustache, les valeurs médianes des séries s'éloignent de plus en 
plus de la distance à la référence.\par
...
\begin{description}
    ...
    perturbation. Cependant, nous constatons que cette méthode devient très imprécise lorsque du bruit
    apparaît devant le capteur.
    \item[Médiane] \hfill \\ 
    Malgré une amélioration par rapport à la moyenne, la médiane 
    est encore beaucoup trop éloignée de la vraie distance au sol. L'erreur est à 
    ...
    que, statistiquement, le capteur finisse toujours par faire au moins une mesure de la distance au sol dans 
    la série.
\end{description}
...
de -20°C lors d'une nuit glaciale jusqu'à 30°C voire 40°C à l'intérieur du boîtier, en plein soleil.
